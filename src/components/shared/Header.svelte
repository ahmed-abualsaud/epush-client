<script>
  ...
  import RouteDisplay from "./RouteDisplay.svelte"
	...
	import AddMessageButton from "../client/AddMessage/AddMessageButton.svelte"
...
<div class="flex flex-col items-start w-full">
  ...
  {#if empty(client) || empty(lastOrder) || client?.balance <= 10 || lastOrder?.number_of_messages <= 10}
    <div class="flex items-center self-stretch p-4 gap-3 border border-warn-200 bg-warn-25">
      ...
      <div class="flex justify-end items-center flex-1 gap-3">
        ...
        <div class="flex items-start gap-3">
          ...
          <button on:click={() => showModal("add-balance")} class="flex justify-center items-center gap-2">
