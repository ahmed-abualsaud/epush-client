<script>
    ...
    import { listMessageGroups, getMessageGroupRecipients } from "../../../../api/messageApi"
    ...
    onMount(async () => {
        ...
        groups = (await listMessageGroups(1000000)).data.map(mapFunction)
    })
    ...
    const selectAll = async () => {
        ...
        for (let i = 0; i < groupsIds.length; i++) {    
            for (let j = 0; j < 1000; j++) {
                recipientsChunk = (await getMessageGroupRecipients(groupsIds[i], j + 1, 5000)).data
    ...
    const selectGroup = async (group) => {
        ...
        allSelected = groupsIds.length === groups.length ? true : false
    }
    ...
    $: {
        ...
        $message.validNumbers = $message.connections.map(conn => conn.numbers).flat()
    }
</script>
