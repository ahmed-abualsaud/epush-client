<script>
	import ExtendedInput from "../../ExtendedInput.svelte"
  ...
  $: {
    ...
    $message.invalidNumbers = inputs.filter((input) => ! empty(input.value)).map((input) => "" + input.prefix + input.value).filter(num => ! $message.validNumbers.includes(num))
