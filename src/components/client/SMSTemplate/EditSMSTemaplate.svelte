<script>
    ...
    import { listMessageLanguages, updateMessageTemplate } from "../../../api/messageApi"
    ...
    const updateTemplate = async () => {
        try {
            ...
            showModal(SMSTemplateSuccessModal, {action: "update"})
			await setcurrentRouteDisplay("Home")
            await setcurrentRouteDisplay("SMS Templates")
        } catch (e) {
            showModal(SMSTemplateErrorModal, {action: "update"})
        }
    }
    ...
    $: {
        let msglen = calcMessageLength(templateContent)
        if (msglen <= selectedLangObj?.max_characters_length) {
            numberOfSegments = 1
        } else if (msglen > selectedLangObj?.max_characters_length) {
            numberOfSegments = splitStringByLength(templateContent, selectedLangObj?.split_characters_length).length
        } else {
            numberOfSegments = 0
        }
    }
...
<Modal>
    <div class="flex flex-col items-center rounded-xl bg-white shadow-xl w-[800px]">
        ...
        <div class="flex flex-col items-start self-stretch gap-4 px-6 pb-8">
            ...
            <div class="flex flex-col items-start self-stretch gap-2">
                ...
                <div class="flex flex-col items-start self-stretch flex-1 gap-1.5">
                    <textarea bind:value={templateContent} on:keydown={(e) => messageLanguageFilter(e, selectedLangObj.characters, selectedLanguage)} placeholder="Enter your message.." class="flex items-start flex-1 self-stretch gap-2 py-3 px-3.5 rounded-lg border border-gray-300 bg-white shadow-sm outline-none min-h-64" />
                    <span class="self-stretch text-gray-600 text-sm">{calcMessageLength(templateContent)}/{(numberOfSegments <= 1 ? selectedLangObj.max_characters_length : numberOfSegments * selectedLangObj.split_characters_length) ?? 0} Characters ({empty(selectedLanguage) ? "English" : selectedLanguage})</span>
                </div>
            </div>
        </div>
        <div class="flex justify-end items-center self-stretch flex-1 gap-3 p-6 border-t border-gray-200">
			...
			<button disabled={empty(templateContent) || empty(templateName)} on:click={updateTemplate} class="flex justify-center items-center gap-2 py-2.5 px-[18px] rounded-lg border border-primary-600 bg-primary-600 shadow-sm {(empty(templateContent) || empty(templateName)) ? "opacity-40" : "opacity-100"}">
