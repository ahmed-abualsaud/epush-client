        ...
        <div class="flex flex-col items-center self-stretch gap-6">
            <div class="flex flex-col items-center self-stretch gap-3">
                <div class="flex flex-col items-center self-stretch gap-1">
                    <span class="self-stretch text-gray-900 text-center text-lg font-semibold">Template {ucFirst(action)} Successful!</span>
                    {#if action == "save"}
                        <span class="self-stretch text-gray-600 text-center text-sm">Your new SMS template has been saved successfully.</span>
                    {:else if action == "update"}
                        <span class="self-stretch text-gray-600 text-center text-sm">Your SMS template changes has been applied and saved successfully.</span>                
                    {:else}
                        <span class="self-stretch text-gray-600 text-center text-sm">Your SMS template has been deleted successfully.</span>
                    {/if}
                </div>
            </div>
        </div>
