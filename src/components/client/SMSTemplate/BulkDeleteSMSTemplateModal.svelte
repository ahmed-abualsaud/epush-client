<script>
    ...
    import { deleteMessageTemplate } from "../../../api/messageApi"
    ...
    const deleteSelectedSMSTemplates = async () => {
        try {
            for (let i = 0; i < templates.length; i++) {
                await deleteMessageTemplate(templates[i].id)
            }

            showModal(SMSTemplateSuccessModal, {action: "delete"})
            await setcurrentRouteDisplay("Home")
            await setcurrentRouteDisplay("SMS Templates")
        } catch (e) {
            showModal(SMSTemplateErrorModal, {action: "delete"})
        }
    }
    ...
    <div class="flex flex-col items-center rounded-xl bg-white shadow-xl w-[480px]">
        <div class="flex flex-col items-center self-stretch gap-4 pt-6 pb-5 px-6 bg-white rounded-t-xl border-b">
            ...
            <div class="flex flex-col items-center self-stretch gap-1">
                <span class="self-stretch text-center text-gray-900 text-lg font-semibold">Are you sure to delete<br>these selected SMS templates?</span>
                <span class="self-stretch text-center text-gray-600 text-sm">You will permanently delete these selected SMS templates</span>
            </div>
        </div>
        <div class="flex flex-col items-start self-stretch gap-3 pt-8 pb-6 px-6">
            <button on:click={deleteSelectedSMSTemplates} class="flex justify-center items-center self-stretch gap-2 py-2.5 px-[18px] rounded-lg border border-error-600 bg-error-600 shadow-sm">
