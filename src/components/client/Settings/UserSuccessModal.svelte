            ...
            <div class="flex flex-col items-center self-stretch gap-3">
                <div class="flex flex-col items-center self-stretch gap-1">
                    {#if avatarAction == "update"}
                        <span class="self-stretch text-gray-900 text-center text-lg font-semibold">Avatar Update Successful!</span>
                        <span class="self-stretch text-gray-600 text-center text-sm">Your avatar has been updated successfully.</span>
                    {:else if avatarAction == "delete"}
                        <span class="self-stretch text-gray-900 text-center text-lg font-semibold">Avatar Delete Successful!</span>
                        <span class="self-stretch text-gray-600 text-center text-sm">Your avatar has been deleted successfully.</span>
                    {:else}
                        <span class="self-stretch text-gray-900 text-center text-lg font-semibold">Information Update Successful!</span>
                        <span class="self-stretch text-gray-600 text-center text-sm">Information changes has been applied and saved successfully.</span>
                    {/if}
                </div>
            </div>
        </div>
