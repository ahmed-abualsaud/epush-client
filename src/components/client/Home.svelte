<script>
    ...
    import { getOrders } from "../../api/expenseApi"
...
<div class="flex flex-col justify-center items-start self-stretch flex-1 gap-6 px-6">
                ...
                <div class="flex flex-col items-start gap-2">
                    ...
                    <div class="flex items-start gap-5 flex-col sm:flex-row">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-gray-900 font-medium">{$lastOrder.number_of_messages ?? "0.0"} SMS</span>
                        </div>
                    </div>
                </div>
                <div class="flex items-center gap-12">
                    <button on:click={() => showModal("add-balance")} class="flex justify-center items-center gap-2 py-2.5 px-4 rounded-lg border border-gray-900 bg-gray-900 shadow-md">
    ...
    <div class="flex items-center self-stretch gap-6">
        <div class="flex flex-col items-start flex-1 gap-5 rounded-xl border border-gray-200 bg-white shadow-sm w-full">
            <div class="flex flex-col items-start self-stretch gap-5 rounded-xl bg-white">
            ...
            {#if shownTable === "messages"}
                <Table hideFilter={true} allowBulkActions={false} {mapFunction} fetchFunction={async () =>  latestMessages(4)} columns={[
                    'sender_name',
                    'message',
                    'sent_date',
                    'type_of_send',
            ...
            {#if shownTable === "orders"}
                <Table hideFilter={true} allowBulkActions={false} mapFunction={(o) => {o.date = readableTimestamp(o.created_at); return o}} fetchFunction={() => getOrders(4)} columns={[
                    'credit',
                    'date'
                ]} />
            {/if}
