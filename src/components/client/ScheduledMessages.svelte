<script>
	...
	import DeleteMessageModal from "./ScheduledMessage/DeleteMessageModal.svelte"
	import PushNowMessageModal from "./ScheduledMessage/PushNowMessageModal.svelte"
	import ViewScheduledMessage from "./ScheduledMessage/ViewScheduledMessage.svelte"
    import BulkDeleteMessageModal from "./ScheduledMessage/BulkDeleteMessageModal.svelte"
	import ChangeScheduleTimeModal from "./ScheduledMessage/ChangeScheduleTimeModal.svelte"
	import BulkChangeScheduleTimeModal from "./ScheduledMessage/BulkChangeScheduleTimeModal.svelte"
	...
	import { 
        ...
        getScheduledCustomMessages,
        getScheduledGroupSMSMessages,
        getScheduledCustomWithParametersMessages
    } from "../../api/messageApi"


    let messageSelector
    let shownTable = "all"
    let noDataMessage =  "You don't have any scheduled message for now!"
    let noDataDescription = "Scheduled messages will appear here."
    ...
    onMount(async () => {
        addSlugToCurrentRouteDisplay("Scheduled Messages", shownTable, 1)
    ...
    const mapFunction = (message, index) => {
        ...
        message.scheduled_date = readableTimestamp(message.scheduled_at)
    ...
    const renderStatus = () => {
        return {
            ...
            props: {
                text: "Scheduled",
                color: "orange",
                icon: '<svg style="padding-right: 4px; width: 12px;" xmlns="http://www.w3.org/2000/svg" width="8" height="8" viewBox="0 0 8 8" fill="none"><circle cx="4" cy="4" r="3" fill="#EF6820"/></svg>'
            }
        }
    }
    ...
    const onPreviewMessage = (message) => {
        showModal(ViewScheduledMessage, {message})
    }

    const onEditMessage = (message) => {
        showModal(ChangeScheduleTimeModal, {message})
    }

    const onDeleteMessage = (message) => {
        showModal(DeleteMessageModal, {message})
    }

    const onPush = (message) => {
        showModal(PushNowMessageModal, {message})
    }

    const onBulkEdit = (messages) => {
        showModal(BulkChangeScheduleTimeModal, {messages})
    }

    const onBulkDelete = (messages) => {
        showModal(BulkDeleteMessageModal, {messages})
    }
</script>
