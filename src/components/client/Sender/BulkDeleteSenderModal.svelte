<script>
    ...
    import { deleteSender } from "../../../api/messageApi"
    import { showModal, hideModal } from "$lib/helper/modal"
    import { getClientSenders } from "../../../api/clientApi"
    import { readableToTimestamp } from "$lib/helper/dateTime"
    import { setcurrentRouteDisplay } from "$lib/router/router"


    export let senders = []

    const deleteSelectedSenders = async () => {
        let sender = {}
        let sndrs = await getClientSenders()

        try {
            for (let i = 0; i < senders.length; i++) {
                sender = sndrs.find(snd => snd.name == senders[i].sender_name && snd.created_at.startsWith(readableToTimestamp(senders[i].created_at).split(" ")[0]))
                if (! empty(sender)) {
                    await deleteSender(sender.id)
                }
            }

            showModal(SenderSuccessModal, {action: "delete"})
            await setcurrentRouteDisplay("Home")
            await setcurrentRouteDisplay("Senders")
        } catch (e) {
            showModal(SenderErrorModal, {action: "delete"})
        }
    }
    ...
    <div class="flex flex-col items-center rounded-xl bg-white shadow-xl w-[480px]">
        <div class="flex flex-col items-center self-stretch gap-4 pt-6 pb-5 px-6 bg-white rounded-t-xl border-b">
            ...
            <div class="flex flex-col items-center self-stretch gap-1">
                <span class="self-stretch text-center text-gray-900 text-lg font-semibold">Are you sure to delete<br>these selected senders?</span>
                <span class="self-stretch text-center text-gray-600 text-sm">You will permanently delete these selected senders</span>
            </div>
        </div>
        <div class="flex flex-col items-start self-stretch gap-3 pt-8 pb-6 px-6">
            <button on:click={deleteSelectedSenders} class="flex justify-center items-center self-stretch gap-2 py-2.5 px-[18px] rounded-lg border border-error-600 bg-error-600 shadow-sm">
