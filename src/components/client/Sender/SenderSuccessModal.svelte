        ...
        <div class="flex flex-col items-center self-stretch gap-6">
            <div class="flex flex-col items-center self-stretch gap-3">
                <div class="flex flex-col items-center self-stretch gap-1">
                    <span class="self-stretch text-gray-900 text-center text-lg font-semibold">Sender {ucFirst(action)} Successful!</span>
                    {#if action == "save"}
                        <span class="self-stretch text-gray-600 text-center text-sm">Your new sender has been saved successfully.</span>
                    {:else if action == "update"}
                        <span class="self-stretch text-gray-600 text-center text-sm">Your sender changes has been applied and saved successfully.</span>                
                    {:else}
                        <span class="self-stretch text-gray-600 text-center text-sm">Your sender has been deleted successfully.</span>
                    {/if}
                </div>
            </div>
        </div>
