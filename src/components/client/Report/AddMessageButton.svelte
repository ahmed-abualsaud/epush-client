<script>
    ...
    const addBulkMessages = () => {
        buttonClicked = ! buttonClicked
        showAddMessageModal(LanguageAndSender, {addMethod: "add-bulk-message"})
    }

    const addCustomMessages = () => {
        buttonClicked = ! buttonClicked
        showAddMessageModal(LanguageAndSender, {addMethod: "add-custom-message"})
    }

    const addCustomWithParameters = () => {
        buttonClicked = ! buttonClicked
        showAddMessageModal(LanguageAndSender, {addMethod: "add-custom-with-parameters"})
    }

    const addGroupSms = () => {
        buttonClicked = ! buttonClicked
        showAddMessageModal(LanguageAndSender, {addMethod: "add-group-sms"})
    }

    const showAddMessageModal = (modalComponent, props) => {
        if (empty($lastOrder)) {
            showModal(NoOrdersModal)
        } else if (empty($currentClient) || $currentClient?.balance == 0) {
            showModal(NoCreditModal)
        } else {
