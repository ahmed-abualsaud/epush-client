    ...
    const onSelectFile = (file) => {
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            for (let j = 0; j < $currentClient.connections.length; j++) {
                if ($currentClient.connections[j].country_code == "20" && number.startsWith("0")) {
                    number = "2" + number
                }

                if ($currentClient.connections[j].country_code == "20" && ! number.startsWith("20")) {
                    number = "20" + number
                }

                if (number.startsWith($currentClient.connections[j].country_code + $currentClient.connections[j].operator_code + "")) {
                    splittedRow[indexOfPhone] = number
                    rows[i] = splittedRow.join(", ")
                    $currentClient.connections[j].numbers.push(rows[i])
        ...
        let validNumbersSet = new Set(validNumbers)
        validNumbers = [...validNumbersSet]
        invalidNumbers = numbers.filter(num => !validNumbersSet.has(num))
    }
