<script>
    ...
    import { deleteMessageGroup } from "../../../api/messageApi"
    ...
    const deleteSelectedGroups = async () => {
        ...
        try {
            ...
            showModal(GroupSuccessModal, {action: "delete"})
            await setcurrentRouteDisplay("Home")
            await setcurrentRouteDisplay("Groups")
        } catch (e) {
            showModal(GroupErrorModal, {action: "delete"})
        }
    }
    ...
    <div class="flex flex-col items-center rounded-xl bg-white shadow-xl w-[480px]">
        <div class="flex flex-col items-center self-stretch gap-4 pt-6 pb-5 px-6 bg-white rounded-t-xl border-b">
            ...
            <div class="flex flex-col items-center self-stretch gap-1">
                <span class="self-stretch text-center text-gray-900 text-lg font-semibold">Are you sure to delete<br>{bulk ? "these selected groups" : "this group"} now?</span>
                <span class="self-stretch text-center text-gray-600 text-sm">You will delete {bulk ? "these selected groups" : "this group"} from<br>your saved groups</span>
            </div>
        </div>
        <div class="flex flex-col items-start self-stretch gap-3 pt-8 pb-6 px-6">
            <button on:click={deleteSelectedGroups} class="flex justify-center items-center self-stretch gap-2 py-2.5 px-[18px] rounded-lg border border-error-600 bg-error-600 shadow-sm">
