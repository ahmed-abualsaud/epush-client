<script>
    ...
    onMount(() => {
        ...
        $message.sendType = "Custom with Parameters"
    ...
    const onSelectFile = (file) => {
        ...
        switch (file.attributes.type) {
            case "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
                rows = [...new Set(parseWordFile(content).filter(n => n && n))]
                break

            case "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet":
                rows = [...new Set(parseExcelFile(content).filter(n => n && n))]
                break

            default:
                rows = [...new Set(parseTextFile(content).filter(n => n && n))]
                break
        }
        ...
        $message.numbersAttributes = empty(rows) ? [] : rows[0].split(/,\s*/).map(key => key.toLowerCase())
        ...
        for (let i = 1; i < rows.length; i++) {
            splittedRow = rows[i].split(/,\s*/)
            number = splittedRow[indexOfPhone]
            for (let j = 0; j < $message.connections.length; j++) {
                if ($message.connections[j].country_code == "20" && number.startsWith("0")) {
                    number = "2" + number
                }

                if ($message.connections[j].country_code == "20" && ! number.startsWith("20")) {
                    number = "20" + number
                }

                if (number.startsWith($message.connections[j].country_code + $message.connections[j].operator_code + "")) {
                    splittedRow[indexOfPhone] = number
                    rows[i] = splittedRow.join(", ")
                    $message.connections[j].numbers.push(rows[i])
                    $message.validNumbers.push(number)
                }
            }
            numbers.push(number)
        }
    ...
    $: if(loadError) {
        errorAlert("Upload numbers failed, Please download sample file to check recipients formatting", 5)
    }
</script>
...
<div class="flex flex-col items-start flex-1 gap-4 w-full">
    <div class="flex items-center self-stretch gap-3">
        ...
        <!-- <button class="flex justify-center items-center gap-2 py-2 px-3.5 rounded-lg border border-gray-300 bg-white shadow-sm">
            <svg class="flex-shrink-0" xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
