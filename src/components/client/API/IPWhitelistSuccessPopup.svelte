...
<Popup name="ipwhitelist-success">
<div class="flex flex-col items-center w-[400px] border bg-white rounded-xl shadow-xl">
        ...
        <div class="flex flex-col items-center self-stretch gap-6">
            <div class="flex flex-col items-center self-stretch gap-3">
                <div class="flex flex-col items-center self-stretch gap-1">
                    {#if action == "update"}
                        <span class="self-stretch text-gray-900 text-center text-lg font-semibold">IP Update Successful!</span>
                        <span class="self-stretch text-gray-600 text-center text-sm">IP changes has been applied and saved successfully.</span>
                    {:else if action == "delete"}
                        <span class="self-stretch text-gray-900 text-center text-lg font-semibold">IP Delete Successful!</span>
                        <span class="self-stretch text-gray-600 text-center text-sm">Your IP has been deleted successfully.</span>
                    {:else}
                        <span class="self-stretch text-gray-900 text-center text-lg font-semibold">IP Save Successful!</span>
                        <span class="self-stretch text-gray-600 text-center text-sm">Your IP has been added successfully.</span>
                    {/if}
                </div>
            </div>
        </div>
    ...
    <div class="flex flex-col items-start self-stretch gap-3 pt-8 pb-6 px-6">
        <button on:click={hidePopup} class="flex justify-center items-center self-stretch gap-2 py-2.5 px-[18px] rounded-lg border border-primary-600 bg-primary-600 shadow-sm">
