...
<div class="flex flex-col items-center w-[400px] border bg-white rounded-xl shadow-xl">
    <div class="flex flex-col items-center self-stretch gap-4 p-6 rounded-t-xl bg-white">
        ...
        <div class="flex flex-col items-center self-stretch gap-1">
            <span class="self-stretch text-gray-900 text-center text-lg font-semibold">Message {deleteMessage ? "Delete" :"Update"} Successful!</span>
            {#if deleteMessage}
            <span class="self-stretch text-gray-600 text-center text-sm">Your scheduled message has been deleted successfully.</span>
            {:else}
            <span class="self-stretch text-gray-600 text-center text-sm">Your message changes has been applied and saved successfully.</span>                
            {/if}
        </div>
    </div>
