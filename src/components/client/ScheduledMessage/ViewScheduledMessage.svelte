...
<div class="flex items-start rounded-xl bg-white shadow-xl h-screen w-[850px]">
    <div class="flex flex-col items-center flex-shrink-0 bg-white rounded-xl my-5 w-full overflow-y-auto" style="height: calc(100vh - 44px);">
        <div class="flex flex-col items-start self-stretch gap-4 pt-1 pb-5 px-6 bg-white rounded-t-xl border-b">
            <div class="flex flex-col items-start self-stretch gap-1">
                ...
                <div class="flex items-start gap-2">
                    <span class="text-gray-700 text-sm font-medium">Scheduled at</span>
                    <span class="text-gray-500 text-sm">{message?.scheduled_date}</span>
                </div>
            </div>
        ...
        {#if ! empty(messageRecipients)}
        <div class="flex items-center self-stretch gap-3 mt-5 pt-5 px-6 bg-white border-t">
            <button on:click={deleteMessage} class="flex items-center">
                <span class="text-error-700 font-semibold">Cancel Message</span>
            </button>
            <div class="flex justify-end items-center self-stretch flex-1 gap-3">
                <button on:click={showCalendar} class="hide-dropdown flex px-3.5 py-2.5 items-center gap-2 self-stretch rounded-lg border border-gray-300 bg-white shadow-sm">
                    <div class="flex items-center flex-1 gap-2 truncate">
                        ...
                        {#if empty(scheduleTime)}
                        <span class="text-gray-900 font-medium truncate">{message.scheduled_at}</span>
                        {:else}
                        <span class="text-gray-900 font-medium truncate">{scheduleTime}</span>
                        {/if}
                    </div>
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                ...
                <button on:click={saveChanges} class="flex justify-center items-center gap-2 py-2.5 px-4 rounded-lg border border-primary-600 bg-primary-600 shadow-sm hover:bg-primary-700">
                    <span class="text-white text-sm font-semibold">Save Changes</span>
                </button>
            </div>
        </div>
        {/if}
