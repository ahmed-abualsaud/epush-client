<script>
    ...
    const nextStep = () => {
        ...
        validNumbers.forEach(number => {
            ...
            messages.push({number: attrValues[$message.numbersAttributes.indexOf("phone")], content: msg, segments: getMessageSegements(msg)})
        })

        $message.messageWithAttributes = messages
        showModal(addMethod + ":date-and-push")
    }

    const back = async () => {
        showModal(addMethod + ":numbers")
    }

    const onSelectTemplate = (templateName) => {
        textareaHtml = messageTemplates.find(template => template.name == templateName)?.template
        $message.segments = getMessageSegements(textareaText)
    }
    ...
    const getMessageSegements = (content) => {
        ...
        if (msglen > $message.language?.max_characters_length) {
            return splitStringByLength(content, $message.language?.split_characters_length)
        }
        return []
    }
</script>

<AddMessageModal modalName={addMethod + ":message"}>
