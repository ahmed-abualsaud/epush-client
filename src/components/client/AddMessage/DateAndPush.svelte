<script>
    ...
    const pushMessage = () => {
        let dateTimeSlected = $message.scheduleTime
        if (empty($message.scheduleTime)) {
            dateTimeSlected = (new Date()).toISOString().substring(0, 19).replace("T", " ")
        }
        showModal("push-success-modal")
    }

    const back = async () => {
        showModal(addMethod + ":message")
    }
...
<AddMessageModal modalName={addMethod + ":date-and-push"}>
